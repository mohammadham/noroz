...
?>
<div style="position: sticky; top: 90px;" class="py-2">
    <form action="<?php echo site_url('blogs'); ?>" method="get" class="w-100">
        <div class="form-floating blog-search-box d-flex">
            <input class="form-control blog-search-input" value="<?php if(isset($search_string) && !empty($search_string)) echo $search_string; ?>" type="text" id="blog_search" placeholder="<?php echo site_phrase('search_blogs'); ?>" name="search" onkeyup="show_submit_button(this)" onblur="show_submit_button(this)">
            ...
            <a href="<?php echo site_url('blogs?category='.$blog_category['slug']); ?>" class="py-3 px-2 list-group-item-action <?php if(isset($_GET['category']) && $_GET['category'] == $blog_category['slug'])echo 'bg-light'; ?>" aria-current="true">
                <div class="d-flex w-100 justify-content-between">
                    ...
                    <?php if($popular_category['blog_number'] > 0): ?>
                        <span class="badge badge-primary badge-pill pt-2"><?php echo $popular_category['blog_number']; ?></span>
                    <?php endif; ?>
                </div>
                <span class="ellipsis-line-2 text-12px"><?php echo $blog_category['subtitle']; ?></span>
            </a>
        <?php endforeach; ?>
        <a class="text-14px pl-2 my-2 text-muted" href="<?php echo site_url('blog/categories'); ?>"><?php echo site_phrase('all_categories'); ?></a>
    </div>
    ...
    <div class="list-group">
        ...
        <?php endforeach; ?>
        <a class="text-14px pl-1 my-2 text-muted" href="<?php echo site_url('blogs'); ?>"><?php echo site_phrase('all_blogs'); ?></a>
    </div>
</div>
